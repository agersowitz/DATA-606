...
```{r plot, eval=TRUE}
plot(mlb11$at_bats, mlb11$runs, main="Scatterplot Example",
...
These two varibales are postively correlated. It is not a very strong relationship but it is noticeable. The variablity tends to increase the more at bats or runs there are.
...
3.  Using `plot_ss`, choose a line that does a good job of minimizing the sum of
    ...
    I plotted on base percentage and on base plus slugging to get a small sum of squares of 0.006. This is logical that these are closely correlated because on base percentage makes up a portion of the formual for on base plus slugging.
    
    ```{r squares, eval=TRUE}
plot_ss(x = mlb11$new_onbase, y = mlb11$new_obs, showSquares = TRUE)
```
...
The Phillies has 5579 at bats and 713 runs.So we have overestimated by about 15 runs.
## Model diagnostics
...
6.  Is there any apparent pattern in the residuals plot? What does this indicate
    ...
    There does not appear to be a patttern to the residual plot which indicates a linear relationship.
...
7.  Based on the histogram and the normal probability plot, does the nearly 
    ...
    Yes, they are not perfectly symmetric but are roughly symetric and unimodal.
...
Yes, the variablity around the least squares line is relatively constant.
...
-   How does this relationship compare to the relationship between `runs` and 
    ...
    Hits predicts runs better than at bats. You can tell because the R^2 is 0.6419 which means 64.2% of the variability in runs can be explained by hits which is much higher than the 37.3% number for atbats.
...
Batting Average best predicts runs. We can tell this because it has the highes R^2 value of 0.6561.
...
The on base plus slugging percentage statistic best predicts run with an R^2 of 0.9349. This makes sense as it incorporates both "moneyball" ideals of getting on base and the current leading strategy oh high homerun and high strike out rates. By being able to blend getting on base with getting as many total bases per at bat as possible you will score the most runs.
...
These plots show that this variable has nearly normal residuals, constant variablity and linearity.
