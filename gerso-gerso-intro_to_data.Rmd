...
1.  How many cases are there in this data set?  How many variables?  For each 
    ...
    There are 9 variables and 20000 cases
    
    genhlth: categorical, ordinal
    exerany: cateogrical, non-ordinal
    hlthpln: categorical, non-ordinal
    smoke100: categorical, non-oirdinal
...
```
There are 9569 males in the study
...
```
0.232 or 23.2% of the population say they are in excellent health.
...
The mosaic plot shows that a higher proportion of men smoke then women.
...
4.  What does this box plot show? Pick another categorical variable from the 
    ...
    This boxplot shows that good health tends to be correlated with a lower BMI. 
    
```{r boxplot-agebmi, eval=TRUE}
bmi <- (cdc$weight / cdc$height^2) * 703
boxplot(bmi ~ cdc$age)
```    
I chose age because I hypothesized that as you age you tend to gain weight. The data shows that there is a slight increase in BMI over time but then there is a decrease when the respondents reach very old age.
...
```
There is a slight positive correlation between weight and desired weight. This makes sense as people have goals that are close to their current weights and generally realistic.
...
-   What type of data is `wdiff`? If an observation `wdiff` is 0, what does 
    ...
    wdiff is a numerical continuous variable. If wdiff is 0 the person doesn't want to change their weight. If the wdiff is positive the person wants to gain weight and if it is negative they want to lose weight.
...
hist(wdiff)
hist(wdiff, breaks = 250)
```

wdiff has a relativelynormal distribution with a narrow spread that is centered at a slightly negative value. This shows most people want to lose weight but their goals are realistic and close to their current weight.
...
```{r boxplot-w-gender, eval=TRUE}
men <- subset(cdc,  cdc$gender == 'm')
women <- subset(cdc,  cdc$gender == 'f')
wdiffm <- men$wtdesire-men$weight
wdiffw <- women$wtdesire-women$weight
summary(wdiffm)
summary(wdiffw)
boxplot(wdiff ~ cdc$gender)
```
Both men and women want to lose weight but women tend to want to lose more weight then men.
...
q1
q3
within_1 <- subset(cdc, weight < q3 & weight > q1)
dim(within_1)
proportion <- 14152/20000
proportion
```

.7076 or 70.76% of weights fall within one standard dxeviaiton of the mean.
