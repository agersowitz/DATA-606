...
```{r male-female-hist, eval=TRUE}
hist(mdims$hgt)
hist(fdims$hgt)
```
    would you compare the various aspects of the two distributions?
    
    The distribution of male heights isnt distributed as widely as the female heights but they both follow a relatively normal distribution.
...
2.  Based on the this plot, does it appear that the data follow a nearly normal 
    ...
    Yes the data seems to follow a nearly normal distribution.
...
```
    ...
    Not all of the points fall on the line but they are very close to the line. The plot of sim_norm is slightly closer to the line then the plot for the real data but it is relatively close.
...
4.  Does the normal probability plot for `fdims$hgt` look similar to the plots 
    ...
    Yes, they all seem relatively similar which would indicate that the female heights are nearly normal.

5.  Using the same technique, determine whether or not female weights appear to 
    come from a normal distribution.
    
    By using the qqnormism function we see that the distribution for the data for female weights is further form the line then it was for height. It also shows that the sims are noticeably closer to the line then the plot of the real data. This would indicate that female weights do not appear to come from a normal distribution.
...
What is the probability that women is over 160 cm?

```{r pnormprobhgt, eval=TRUE}
...
What is the probability that women is under 60 kg?

```{r pnormprobwgt, eval=TRUE}
...
Height has the closer agreement between the two methods which indicates it is more normally distributed.
...
-   Note that normal probability plots C and D have a slight stepwise pattern.  
    ...
    These plots are for age and elbow diameter. They may have a stepwise pattern because they aren't normally distributed and are skewed. This will lead to more drastic changes as you move along the distribution.
...
-   As you can see, normal probability plots can be used both to assess 
    ...
    Based on the probability plot knee diameter is not normally distributed and it is skewed to the right. You can see this because it is close to the line at the lower and mid diameters but strays form the line the higher it goes.
