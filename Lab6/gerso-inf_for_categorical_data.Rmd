...
We assume the sample was randomly selected, the observations are independent of one another and that the sample size is large enough. The one area where this assumption may be unreasonable is the sample size which can only be an extremely small proporiton of the global population.
...
3.  What does each row of Table 6 correspond to? What does each row of 
    ...
    Each row of table 6 corresponds to the survey results for a specific country. Each row in the atheism dataset corresponds to an individuals response to the survey.
...
4.  Using the command below, create a new dataframe called `us12` that contains
    ...
    Yes it agrees at 4.99%

```{r us-atheism, eval=TRUE}
us12 <- subset(atheism, nationality == "United States" & year == "2012")
head(us12,10)
table(us12$response)
round((50/(952+50))*100,2)
```
...
np=1002(.05)= 50.1       n(1-p) = 1002(1-.05)=951.9
...
7.  Using the `inference` function, calculate confidence intervals for the 
    ...
    Conditions of inference are met for both Brazil and Sweden are met.
    Sweden margin of error = 0.0239, Brazil margin of error = 0.0043.

```{r c-atheism, eval=TRUE}
swe12 <- subset(atheism, nationality == "Sweden" & year == "2012")
#head(swe12,10)
bra12 <- subset(atheism, nationality == "Brazil" & year == "2012")
#head(bra12,10)
inference(swe12$response, est = "proportion", type = "ci", method = "theoretical", 
          success = "atheist")
...
The relationship has a quadratic curve. As the population proportion increases so does the margin of error until you reach half of the population then the margin of error begins to decrease again.
...
The sampling distribution follows a normal distribution. The mean and median are extremely close at 0.099 for the median and .100 for the mean. The spread is spretty small and the shape is a normal distribuiton shape. 
...
Increasing the sample size decreases the spread of the distribution while decreasing the p value causes the center of the distribution to shift left and get smaller.
...
No, one of equadors success-failiure conditions is not met because the number of "successes" is below 10.However, as stated above 10 is arbatrary so continuing iwth the survey and including all results regardless of this metric is understandable.
...
-   Answer the following two questions using the `inference` function. As 
    ...
    *Hint:* Create a new data set for respondents from Spain. Form
    ...
    H0: The proportion of atheists in spain did not change from 2005 to 2012
    HA: The number of atheists in Spain changed from 2005 to 2012
    
    There is a strong overlap between the 95% confidence intervals for spain in 05 ( 0.083 , 0.1177 ) and 12( 0.0734 , 0.1065 ). Additonally, both means from the samples fall within the others confidence interval so we cannot reject the null hypothesis and determine that the rate of atheism has not changed.
    
    
    
    ```{r sp-atheism, eval=TRUE}
...
```
    ...
    **b.** Is there convincing evidence that the United States has seen a
    change in its atheism index between 2005 and 2012?
     ...
     H0: The proportion of atheists in the US did not change from 2005 to 2012
    HA: The number of atheists in the US changed from 2005 to 2012
    
    There is no overlap between the 95% confidence intervals for the US in 05 ( 0.0038 , 0.0161 ) and 12( 0.0364 , 0.0634 ). Additonally, neither means from the samples fall within the others confidence interval so we can reject the null hypothesis and determine that the rate of atheism has changed.
    

    ```{r usa-atheism, eval=TRUE}
...
-   If in fact there has been no change in the atheism index in the countries 
    ...
    *Hint:* Look in the textbook index under Type 1 error.
    ...
    I would anticipate finding a change in about 10% of them just by chance (assuming a two tailed change). Decreasing the significance level to avoid Type 1 error.
...
-   Suppose you're hired by the local government to estimate the proportion of 
    ...
    *Hint:* Refer to your plot of the relationship between $p$ and margin of 
    ...
    You would want to have an extremely small (1%-3% of population) dataset or an extremely large dataset(97%-100% of population). Since it would be extremely difficult to acquire such a large sampel the ideal sample size to be in the guidlines of this request you would need to have a sample that is between 1%-3% of the population. 
